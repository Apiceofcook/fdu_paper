% 第五章：世界知识
\chapter{过滤、增强与反向找图协同的数据合成框架：世界知识}
\label{chap:world_knowledge}

\section{背景}
承接第\ref{chap:gui}章，本文已经在GUI这一“高分辨率、控件密集、状态易变”的场景中，通过\textbf{结构化初始化}与\textbf{像素级可执行监督}（bbox/OCR/关系/状态等）提升了Referring与Grounding等基础能力，并形成了一个简单但可控多样的正向合成框架。然而，GUI任务更多关注\textbf{可见语义与几何定位}（看得准、点得稳），对“图像背后隐含的事实、概念与关系”缺乏直接而可验证的监督，难以覆盖开放世界场景中普遍存在的\textbf{先识别再调用知识、再条件筛选}的推理链路。本章进一步面向世界知识能力提升：在延续前一章数据工程范式的同时，将监督重心推进到\textbf{知识级对齐可信度}与\textbf{推理过程可学习}，并通过类别框架、caption中间表征、相关性过滤、增强以及从知识实体反向寻找图片等机制，把分散知识转化为可学习、可校验的多模态训练信号，进而形成一个\textbf{过滤、增强与反向找图协同的数据合成框架}。
...
\section{升级框架：世界知识数据构造Pipeline}
世界知识增强的关键在于“把分散知识变成可学习的多模态监督”。本文设计了如图\ref{fig:wk_data_pipeline}所示的世界知识数据构造Pipeline，并将其总结为一个由\textbf{类别组织、反向找图、caption中间表征、分数化过滤、任务化生成与过程性增强}共同构成的升级框架。该流程包含词条生成与图片收集、图片Caption生成、图文相关性过滤、QA对生成与QA对质量过滤等核心环节，形成从知识源到可训练数据的完整转化路径。
...
\subsection{词条生成与图片收集：从知识域反向寻找视觉证据}
对应图\ref{fig:wk_data_pipeline}中的“词条生成和图片收集”环节，本文首先在前述知识类别框架的约束下，为每个子类生成可控的词条集合，并围绕词条自动化收集图片，以形成“实体$\leftrightarrow$视觉证据”的锚点，为后续图文对齐与知识学习打下基础。与GUI场景中以真实截图为起点的正向构造不同，这一步本质上是\textbf{从知识实体反向寻找图片}：先确定知识目标，再反向组织视觉证据，从源头提升知识覆盖的可控性与可追溯性：
\begin{itemize}
...
\end{itemize}
在这一阶段，数据工程的目标是“\textbf{实体可控}”与“\textbf{图像高质量}”：实体可控保证知识监督可追溯，图像高质量保证视觉侧对齐不被噪声主导。这也是本章相较GUI框架的一个关键升级点，即不再只是围绕已有图像做正向构造，而是从知识目标反向组织视觉证据。

\subsection{图片Caption生成：融合视觉描述与知识背景}
对应图\ref{fig:wk_data_pipeline}中的“图片CAPTION生成”环节，本文为每张自动抓取的图片生成结构化caption，其作用不仅是“生成更丰富的描述”，更是为后续多个关键环节提供\textbf{可依赖的中间表征}：\textbf{(1) 图文相关性过滤}需要caption帮助模型判断图片是否真的与词条匹配；\textbf{(2) 知识要素提取}可从caption中抽取实体属性、别名与背景事实；\textbf{(3) QA对生成}可将caption作为问题构造与答案验证的上下文依据。换言之，caption不是附加信息，而是本章升级框架中连接“知识实体--视觉证据--训练监督”的关键中间层。
...
对应图\ref{fig:wk_data_pipeline}中的“图文相关性过滤”环节，本文通过以下方式保证数据质量。该步骤也是本章框架相较GUI框架的重要增强点，因为其目标不只是去噪，而是主动在覆盖率与对齐可信度之间取得平衡：
\begin{itemize}
...
为量化“相关性分数”策略对数据\textbf{保留率}与\textbf{幻觉噪声}的影响，本文从原始抓取结果中随机抽取2000条“图片--词条”样本进行对比实验。若不设置相关性分数、仅让模型输出“相关/不相关”的二分类判断，则最终只能保留约1009条数据；而在制定1$\sim$5相关性分数并保留3分及以上样本后，可保留约1511条数据，显著提高可用数据覆盖。

进一步地，本文对上述2000条样本及其不同过滤策略后的保留子集进行人工抽检，主要关注上一步生成的caption在对照词条与图片时是否出现知识性幻觉（例如模型不认识/误识别实体，从而编造与词条不符的背景事实）。本文将幻觉程度划分为三档：\textbf{几乎没有幻觉}、\textbf{有部分幻觉}、\textbf{大部分幻觉}。统计结果如表\ref{tab:wk_rel_filter_hallu}所示。
...
\begin{tabular}{lcccc}
\toprule
\textbf{数据设置} & \textbf{样本数} & \textbf{几乎无幻觉} & \textbf{部分幻觉} & \textbf{大部分幻觉} \\
\midrule
...
可以看到，\textbf{相关性分数的制定}在保留更多数据的同时，“几乎无幻觉”样本占比也显著上升，并有效降低“部分/大部分幻觉”样本比例。这说明分数化判别相比二分类更能缓解因大模型自身不认识或误识别带来的误判与幻觉噪声，从而在覆盖率与对齐质量之间取得更优平衡，也说明“caption中间表征 + 分数化过滤”是本章框架能够稳定提升数据可用性的核心模块。

\subsection{QA对生成：从描述到三元组任务}
对应图\ref{fig:wk_data_pipeline}中的“QA对生成”环节，为了解耦感知与推理，本文构造三类互补问答对。它们不仅对应不同能力层次，也构成了本章升级框架中的任务化生成模块：
\begin{itemize}
  \item \textbf{RecQA（Recognition QA）}：对应“模型\textbf{看见}图片”的能力，问题围绕“这是谁/这是什么/来自哪里”等识别任务，答案为实体或关键属性。其目标是将视觉特征稳定锚定到词条实体，解决“看到但叫不出名字”的问题。在生成时，模型输入包括图片、图片对应的caption、目标词条以及词条所属类别，用于约束识别问题的指向性与答案空间。
  \item \textbf{KnowQA（Knowledge QA）}：对应“模型\textbf{知道}关于该实体的世界知识”的能力，问题强调外部知识（历史、文化、科学属性等），形式上可不依赖视觉信息，但与图片实体强相关。其目标是向模型注入可泛化的背景事实，解决“认识了但不了解”的问题。在生成时，模型输入为RecQA问答对与caption，并要求其\textbf{基于RecQA的答案（已识别实体）}提出相关知识问题，从而将知识提问锚定在正确实体上。
  \item \textbf{FinalQA（Reasoning QA）}：对应“模型\textbf{理解}图片并能调用知识推理”的能力，问题需要“先识别再用知识推理”，可是一跳或多跳链路（如识别实体$\rightarrow$回忆知识$\rightarrow$条件筛选）。其目标是打通RecQA与KnowQA形成的知识链路，推动模型从“描述/识别”走向“解释/推断”。在生成时，模型输入同时包含RecQA与KnowQA两类QA，引导其融合“识别结论+知识要点”构造最终问题与答案，显式贯通两阶段信息。
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{评测基准去重（hash去重，避免数据泄漏）}：为避免训练数据与评测基准（如Chinese SimpleVQA、SimpleVQA）在图像层面发生重叠导致评测泄漏，本文在构造训练集时对评测基准中的图片进行hash去重：对训练集与评测集图片分别计算感知哈希（perceptual hash，如pHash/dHash等），并以哈希值作为指纹建立索引；若训练样本图片与评测基准图片\textbf{哈希完全一致}，则直接从训练集中剔除；同时可进一步基于哈希间的\textbf{汉明距离}进行相似度判别，在距离小于阈值时视为近重复（如压缩、裁剪、分辨率变化导致的轻微差异）并一并过滤。该步骤从源头保证评测的独立性与结论可信度。
\end{itemize}

\section{训练与实验：从数据类型消融到CoT对齐}
本节将训练策略与实验分析合并阐述：首先通过消融实验验证不同数据类型对世界知识能力的影响；随后针对推理指标“增益不显著甚至下降”的现象，引入带推理过程的Think/CoT数据并扩充数据分布，给出改进后的结果；最后报告在\textbf{Chinese SimpleVQA}与\textbf{SimpleVQA}上的最终性能，并分析与强基线之间的差距。整体上，本节更关注各模块增益如何共同支撑本章升级框架的有效性。
...
同时，在同一批次的数据构造流水线中，本文也会针对不同环节的侧重点选用不同模型，以进一步降低单一模型带来的偏置风险：词条生产依赖知识覆盖更强的文本模型\texttt{gpt5}；caption生成使用当时更强的多模态模型\texttt{Gemini-2.5-pro}与\texttt{gpt-4o}以提升视觉描述与背景知识补全质量；后续RecQA/KnowQA/FinalQA的生成更偏向于从caption中抽取与融合知识，因而采用指令遵循更强且生成效率更高的\texttt{Gemini-2.5-flash}以提升规模化产出稳定性。
...
\begin{itemize}
  \item \textbf{与强基线仍有差距}：尽管数据增强带来提升，但与qwen2.5-VL-7b相比\citep{bai2025qwen25vl}，各项指标仍存在明显差距，说明世界知识能力提升仍受限于模型容量、数据覆盖与对齐质量等因素。
...
\subsection{实验二：数据增强}
由于实验一中推理指标不升反降，我们进一步验证本章框架中\textbf{数据增强模块}的作用。核心观察是：当FinalQA仅监督最终答案时，模型更容易“记答案”；而当在框架的数据增强环节中为FinalQA引入简要推理过程（Think/CoT）\citep{wei2022chain}时，原本的Direct Answer样本被增强为“识别$\rightarrow$知识调用$\rightarrow$答案生成”的过程性样本，模型因此被迫先完成识别再调用知识，从而更有效地提升final相关指标。换言之，这里的提升并不仅仅来自“加了一种新标注形式”，而是来自框架在数据增强阶段显式提高了训练信号的推理密度。

基于这一发现，我们围绕框架中的数据增强与数据扩充分别采取如下改进思路，并在表\ref{tab:wk_think_improve}中给出结果：
\begin{itemize}
  ...
  \item \textbf{FinalQA增加CoT过程}：将FinalQA从Direct Answer升级为带简要推理过程的形式；从框架视角看，这一步对应在数据增强环节中显式注入推理步骤，使模型学习“先识别再推理”的过程性能力；
  \item \textbf{增加KnowQA与FinalQA密度}：同一张图片可生成多个KnowQA与FinalQA样本，提高知识链路覆盖与训练信号密度；从框架视角看，这一步同样属于数据增强，用于提升同一视觉证据上的监督密度与问题多样性。
\end{itemize}
...
从表\ref{tab:wk_think_improve}可见，\textbf{引入Think/CoT后的增强数据对final相关指标提升显著}，且在进一步扩充数据分布（Part123）后，recognition与final均获得同步大幅提升。这说明：世界知识能力的提升不仅依赖数据规模与覆盖，更依赖\textbf{训练信号的结构}——当框架在数据增强环节中把原本仅监督最终答案的FinalQA增强为带过程的Think/CoT样本后，模型学习到的不再是训练分布上的“答案记忆”，而是“先识别、再调用知识、再完成推理”的可迁移策略。因此，这里的收益应主要归因于本章框架中\textbf{数据增强模块}的作用：一方面通过Think/CoT增强显式提高推理密度，另一方面通过多问一图与未见词条扩充提高监督覆盖，最终共同体现为框架在“过滤 + 中间表征 + 增强 + 过程监督”逐步完善后的系统性增益。

\subsection{最终结果：Chinese SimpleVQA与SimpleVQA}
在完成上述训练与数据构造策略后，一共产出了\textbf{80w}条数据，在Chinese SimpleVQA与SimpleVQA上的最终结果，分别见表\ref{tab:wk_final_chinese}与表\ref{tab:wk_final_english}。需要说明的是，最终表中的部分指标相较表\ref{tab:wk_think_improve}有所回落，这并不意味着前述策略失效，而是由于两者对应的训练设置不同：表\ref{tab:wk_think_improve}展示的是\textbf{仅使用合成世界知识数据进行SFT}的结果，更偏向对该专项能力的定向优化；而最终结果采用的是\textbf{面向全部能力、混合全部数据的统一SFT}设置，在多任务、多分布联合训练下，世界知识相关能力会受到一定的目标稀释与任务干扰。总体上，Ovis2.5\_9B在中文基准上对recognition指标达到并超过部分基线，但在final推理指标上与更大模型（如Qwen2.5-VL-72B）仍存在差距，说明后续仍可从模型容量、检索增强或更高质量推理数据等方向继续提升。
...
\section{本章小结}
本章围绕视觉语言模型在世界知识理解上的不足，系统构建了面向世界知识增强的数据体系与训练框架，并将其总结为过滤、增强与反向找图协同的数据合成框架，为后续统一框架的提出提供了可复用的工程化基础。本文的主要贡献与创新点可概括为：
\begin{itemize}
  \item \textbf{提出可操作的世界知识类别框架}：构建7个大类、40个子类的分层分类体系，并强调每个类别/子类的覆盖范围、排除规则与命名规范，从而将“世界知识”从零散事实组织为可采样、可统计、可扩展的结构化坐标系，直接服务于后续反向找图、词条生成与长尾覆盖。
  \item \textbf{给出过滤、增强与反向找图协同的端到端Pipeline}：围绕“词条生成$\rightarrow$自动化图片收集$\rightarrow$caption生成$\rightarrow$相关性过滤$\rightarrow$QA对生成$\rightarrow$质量过滤”建立稳定流水线。尤其通过从知识实体反向寻找图片、caption作为中间表征支撑图文相关性判别，并引入1$\sim$5分相关性评分机制，在覆盖率与噪声控制之间取得更稳健的平衡。
  ...
  \item \textbf{通过消融实验揭示框架升级的有效性}：实验表明caption对识别指标具有稳定增益，而仅提供Direct Answer形式的FinalQA对推理指标提升不显著甚至下降，反映出FinalQA与RecQA在训练中存在短路映射带来的冲突；引入Think/CoT并扩充未见词条与多问一图数据后，final相关指标得到显著改善，说明过程性监督同样是该框架带来的关键增益，并在Chinese SimpleVQA与SimpleVQA上取得具有竞争力的最终结果。
\end{itemize}

对比第\ref{chap:gui}章以\textbf{高精度、可执行、强约束}的像素级监督为核心、侧重“看得准、点得稳”的正向合成框架，本章进一步将监督重心推进到\textbf{知识级对齐可信度}与\textbf{推理过程可学习}：通过知识类别框架与反向找图实现可控覆盖，以caption作为可依赖的中间表征支撑相关性分数过滤与对齐校验，并以RecQA/KnowQA/FinalQA（及Think/CoT）将“先识别再推理”的链路显式化，从而在保留更多数据的同时抑制幻觉与短路学习。上述实践并非与GUI框架并列的另一套零散方案，而是在其基础上的进一步完善，沉淀出跨任务可复用的共通骨架（数据源组织、结构化初始化、任务分解与生成、增强/过滤与多维校验、回流迭代）。基于这些共通结构，下一章将进一步抽象并系统化提出完整的Auto-Evol多模态合成数据构造框架，将零散的一次性合成升级为可路由、可评测、可闭环迭代的数据生产系统。
