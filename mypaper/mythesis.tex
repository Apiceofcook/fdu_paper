...
多模态大语言模型在通用视觉理解任务上快速发展，但训练数据普遍存在噪声大、对齐弱与推理密度不足等问题，使得“数据质量”逐渐成为能力提升瓶颈。本文首先从两类高难真实需求出发完成方法落地：其一，面向移动端用户界面使用场景，构建多源融合与精标语料，并结合高分辨率感知与训练策略提升理解与定位能力，在前200主流应用评测中取得显著提升；其二，面向世界知识，建立知识类别框架与端到端数据流水线构造框架，构造了三种不同类型的问答对并引入Think/CoT过程监督，增强“先识别再推理”的推理能力。基于上述实践与实验归因，本文进一步抽象并提出Auto-Evol多模态合成数据构造框架，将数据生产从单步“图像$\rightarrow$文本/问答”生成升级为闭环系统：通过结构化初始化、任务路由与原子问题生成、代理式增强、过滤校验与回流重写，稳定产出高信息密度、高对齐度与高推理密度的训练信号。用户移动界面理解与世界知识两条任务线的量化结果为该框架中“结构化锚点/中间表征/过程性监督/评测式质控”等关键设计提供了直接证据，表明该框架与数据工程方法可有效提升模型在真实场景中的定位、理解与知识推理表现，并为后续扩展提供通用框架。
