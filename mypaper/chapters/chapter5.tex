...
\section{背景}
承接第\ref{chap:gui}章，本文已经在 GUI 这一“高分辨率、控件密集、状态易变”的场景中，通过\textbf{结构化初始化}与\textbf{像素级可执行监督}（bbox/OCR/关系/状态等）提升了 Referring 与 Grounding 等基础能力，验证了“以数据工程前置约束训练信号、从源头控制噪声”的有效性。然而，GUI 任务更多关注\textbf{可见语义与几何定位}（看得准、点得稳），对“图像背后隐含的事实、概念与关系”缺乏直接而可验证的监督，难以覆盖开放世界场景中普遍存在的\textbf{先识别再调用知识、再条件筛选}的推理链路。沿第1章“\textbf{数据能力光谱}”的研究路径，本章从光谱的\textbf{感知端}转向\textbf{推理端}：在延续数据工程范式的同时，将监督粒度从“像素/字段级”上升到“实体/事实级”、训练目标从“看得准、点得稳”扩展到“懂得对、推得通”，并通过类别框架、caption 中间表征、相关性过滤与三类 QA 任务分解，把分散知识转化为可学习、可校验的多模态训练信号；本章沉淀的可验证对齐、过程性推理监督与中间表征（caption）等模块，将与第\ref{chap:gui}章在感知端的实践共同构成第5章 Auto-Evol 统一框架的实证基础。
