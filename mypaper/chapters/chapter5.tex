...
\section{背景}
承接第\ref{chap:gui}章，本文已经在GUI这一“高分辨率、控件密集、状态易变”的场景中，通过\textbf{结构化初始化}与\textbf{像素级可执行监督}（bbox/OCR/关系/状态等）提升了Referring与Grounding等基础能力，验证了“以数据工程前置约束训练信号、从源头控制噪声”的有效性。然而，GUI任务更多关注\textbf{可见语义与几何定位}（看得准、点得稳），对“图像背后隐含的事实、概念与关系”缺乏直接而可验证的监督，难以覆盖开放世界场景中普遍存在的\textbf{先识别再调用知识、再条件筛选}的推理链路。本章进一步面向世界知识能力提升：在延续数据工程范式的同时，将监督重心推进到\textbf{知识级对齐可信度}与\textbf{推理过程可学习}，并通过类别框架、caption中间表征、相关性过滤与三类QA任务分解，把分散知识转化为可学习、可校验的多模态训练信号。
