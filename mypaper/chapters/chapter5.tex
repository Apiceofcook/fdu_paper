...
\section{背景}
承接第\ref{chap:gui}章，本文已经在 GUI 这一“高分辨率、控件密集、状态易变”的场景中，通过\textbf{结构化初始化}与\textbf{像素级可执行监督}（bbox/OCR/关系/状态等）提升了 Referring 与 Grounding 等基础能力，验证了“以数据工程前置约束训练信号、从源头控制噪声”的有效性。然而，GUI 任务更多关注\textbf{可见语义与几何定位}（看得准、点得稳），对“图像背后隐含的事实、概念与关系”缺乏直接而可验证的监督，难以覆盖开放世界场景中普遍存在的\textbf{先识别再调用知识、再条件筛选}的推理链路。沿第1章“\textbf{数据能力光谱}”的研究路径，本章从光谱的\textbf{感知极端}转向\textbf{推理极端}：在延续数据工程范式的同时，将监督粒度从“像素/字段级”上升到“实体/事实级”、训练目标从“看得准、点得稳”扩展到“懂得对、推得通”，并通过类别框架、caption 中间表征、相关性过滤与三类 QA 任务分解，把分散知识转化为可学习、可校验的多模态训练信号；本章沉淀的可验证对齐、过程性推理监督与中间表征（caption）等模块，将与第\ref{chap:gui}章在感知极端的实践共同构成第5章 Auto-Evol 统一框架的实证基础。
...
\section{知识类别框架}
世界知识增强的首要前提，是建立一个\textbf{可覆盖、可扩展、可操作}的知识类别框架。若缺乏统一的类别坐标系，世界知识将以“零散事实”的形式分布在不同来源中，不仅难以进行系统性采样与补全，也难以在训练与评测中对模型能力进行可解释的归因分析。因此，本文构建分层分类体系，包含7个大类与40个子类（如图\ref{fig:wk_class}所示），用于约束数据构造过程并为后续框架化的数据Pipeline提供统一接口。
...
可以看到，\textbf{相关性分数的制定}在保留更多数据的同时，“几乎无幻觉”样本占比也显著上升，并有效降低“部分/大部分幻觉”样本比例。这说明分数化判别相比二分类更能缓解因大模型自身不认识或误识别带来的误判与幻觉噪声，从而在覆盖率与对齐质量之间取得更优平衡。
...
同时，在同一批次的数据构造Pipeline中，本文也会针对不同环节的侧重点选用不同模型，以进一步降低单一模型带来的偏置风险：词条生产依赖知识覆盖更强的文本模型\texttt{gpt5}；caption生成使用当时更强的多模态模型\texttt{Gemini-2.5-pro}与\texttt{gpt-4o}以提升视觉描述与背景知识补全质量；后续RecQA/KnowQA/FinalQA的生成更偏向于从caption中抽取与融合知识，因而采用指令遵循更强且生成效率更高的\texttt{Gemini-2.5-flash}以提升规模化产出稳定性。
...
由于实验一中推理指标不升反降，我们进一步验证\textbf{推理过程监督}的作用。核心观察是：当FinalQA仅监督最终答案时，模型更容易“记答案”；而当FinalQA要求输出简要推理过程（Think/CoT）\citep{wei2022chain}，模型被迫先完成识别再调用知识，从而更有效地提升final相关指标。
...
\begin{itemize}
  ...
  \item \textbf{给出端到端的世界知识数据构造Pipeline}：围绕“词条生成$\rightarrow$自动化图片收集$\rightarrow$caption生成$\rightarrow$相关性过滤$\rightarrow$QA对生成$\rightarrow$质量过滤”建立稳定Pipeline。尤其通过caption作为中间表征支撑图文相关性判别，并引入1$\sim$5分相关性评分机制，在覆盖率与噪声控制之间取得更稳健的平衡。
...
对比第\ref{chap:gui}章以\textbf{高精度、可执行、强约束}的像素级监督为核心、侧重“看得准、点得稳”的数据工程范式，本章进一步将监督重心推进到\textbf{知识级对齐可信度}与\textbf{推理过程可学习}：通过知识类别框架与反向对齐思路实现可控覆盖，以caption作为可依赖的中间表征支撑相关性分数过滤与对齐校验，并以RecQA/KnowQA/FinalQA（及Think/CoT）将“先识别再推理”的链路显式化，从而在保留更多数据的同时抑制幻觉与短路学习。上述实践不仅丰富了相较GUI更“先进”的增强、过滤与过程性约束模块，也沉淀出跨任务可复用的共通骨架（数据源组织、结构化初始化、任务分解与生成、增强/过滤与多维校验、回流迭代）。基于这些共通结构，下一章将进一步抽象并系统化提出完整的Auto-Evol多模态数据合成框架，将零散的一次性合成升级为可路由、可评测、可闭环迭代的数据合成系统；并在框架抽象之上，将Auto-Evol进一步实例化到GUI计算机使用智能体（Computer Use Agent, CUA）场景，依托OSWorld评测体系\citep{xie2024osworld}构建面向\textbf{可验证任务合成}的工程系统，跨LibreOffice三个domain（Calc/Impress/Writer）与VSCode、Chrome等扩展domain验证框架在“看--懂--做”闭环下的可迁移性，并与EvoCUA\citep{xue2026evocua}等代表性工作形成方法学层面的互补（详见第\ref{sec:auto_evol_gui_cua}节）。
